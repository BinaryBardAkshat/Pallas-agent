...
class PromptBuilder:
    ...
    def build_system_prompt(self, context: str = "") -> str:
        base_prompt = (
            ...
            "3. When a user asks you to create a file, explore a directory, install a dependency, or run code, DO NOT tell them how to do it. ACTUALLY EXECUTE the command using your `terminal` or `file` tool.\n"
            "4. Be confident. Assume actions are allowed (they will be caught by a human-in-loop approval gate if necessary). Do not prompt the user for obvious parameters if you can synthesize them yourself."
        )
        if context:
